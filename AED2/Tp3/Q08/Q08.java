...
class ArvoreTrie 
{
   ...
   public void merge(ArvoreTrie resp , ArvoreTrie av)throws Exception
   {
     merge("" , resp , av.raiz);
   }

  public void merge(String s, ArvoreTrie resp , No no)throws Exception 
  {
    if(no.folha == true)
    {
      //MyIO.println(">>>1" + s.trim());
      //MyIO.println(">>>2" + no.elemento);
      String sub = s + no.elemento;
      //MyIO.println(">>>3" + sub.substring(0));
      resp.inserir(sub.substring(1));
    } else 
    {
      for(int i = 0; i < no.prox.length; i++)
      {
        if(no.prox[i] != null)
        {
          merge(s + no.elemento, resp , no.prox[i]);
        }
      }
    }
  }// fim merge 
...
public class Q08
{
   public static void main (String [] args) throws Exception
     { 
       ...
       while(!arqTmp.contains("FIM") )
       {
          resp = a3.pesquisar(arqTmp);
          if(resp == false)
          {
          	MyIO.println("N"+(char)65533 +"O");
          }
          else
          {
          	MyIO.println("SIM" );
     ...
     try
     {
        FileWriter matri = new FileWriter("matrícula_arvoreTrie.txt");
        PrintWriter print = new PrintWriter(matri);
        print.println("553364\t" + tempoFinal + "\t" + a1.getComparacoes()); 
        matri.close(); 
    }//fim try
