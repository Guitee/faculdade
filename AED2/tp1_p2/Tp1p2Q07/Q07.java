...
class Lista 
{
   ...
   public boolean pesquisar(String si)
   {
      ...
      {
      	retorno = ( si.equals(array[i].getNome()));
      }
      ...
      int dir = n -1, esq = 0, meio;
        while (esq <= dir) 
        {
           meio = (esq + dir) / 2;
            ...
            {
                resp = true; // nao estava entrando no true, mudei o no quick
                //MyIO.println("Hello guite"); 
                esq = n ;
            } 
            else if (x.compareTo(array[meio].getNome())  > 0 ) 
            {
                esq = meio + 1;
            } else 
            {
   ...
   public void quickSort(int left, int rigth) 
   {
    ...
    String pivo = array[(rigth + left)/ 2].getNome(); // pegar o meio 

    while (i <= j) 
    {
      while ((array[i].getNome()).compareTo( pivo ) < 0 ) // estava= 0
      {
         i++;
      }   
      while ((array[j].getNome()).compareTo( pivo )  > 0 ) // estava < 0
      {   
         j--;
      }   
      if (i <= j)
  ...
  public void quickSort() // sera um construtor pro quickSort
  {   
      quickSort(0, n-1);  
  } //fim quiSort inicial 
 }//fim class Lista
 ...
 public class Q07
 {
     ...
     { 
       ...
       boolean flag;
       String resp [] = new String [150];
       int cont = 0;
       lista.quickSort(); 
       lista.setComparacoes(0);
       //MyIO.setCharset("ISO-8859-1");
      while(buf.contains("FIM") == false )// enquanto buf != FIM  
      { 
         flag = lista.pesquisaBinaria(buf);
         ...
         cont++; 
         buf = MyIO.readLine(); // ler a proxima linha
       }//fim while
     ...
     try
     {
        FileWriter matri = new FileWriter("matrícula_binaria.txt");
        PrintWriter print = new PrintWriter(matri);
        print.println("553364\t" + tempoFinal + "\t" + lista.getComparacoes()); 
        matri.close(); 
    }//fim try
