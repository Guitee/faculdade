...
class Lista 
{
   ...
   { 
      for(int i = (h + cor); i < n; i+=h)
      {
         ...
         while((j>=0) && resp && (array[j].getPagina().compareTo(tmp.getPagina()) >=0 )) 
         {
            this.setComparacoes(this.getComparacoes()+1);

            if((array[j].getPagina().compareTo(tmp.getPagina()) > 0 )) // se o primeiro for maior que o segundo 
            {
                array[(j+h)] = array[j];
            ...
            else // se o primeiro for menor que o segundo para a repeticao
            {
               resp = false;  
            }   
                        
         }
         array[j+h] = tmp; // recbe o novo valor do tmp
         this.setMovimentacoes(this.getMovimentacoes()+1);  
      }     
...
public class Q11
 {
     ...
     try
     {
        FileWriter matri = new FileWriter("matrícula_shellSort.txt");
