 ...
 class Lista 
{
   ...
   // pega o maior elemento do array e retorna 
    public int getMaior() 
    {
       int maior = array[0].getIdade();
        ...
        for (int i = 0; i < n; i++) 
        {
           if(maior < array[i].getIdade() )
           {
               this.setMovimentacoes(this.getMovimentacoes()+1);
              maior = array[i].getIdade();
           }
        }
       return maior;    
    }//fimm getMaior()
   ...
   //Algoritmo de ordenacao Countingsort.
    public void countingSort() 
    {
      ...
      for (int i = 0; i < n; count[array[i].getIdade()]++, i++);
      ...
      //Ordenando
      for(int i = n-1; i >= 0; ordenado[count[array[i].getIdade()]-1] = array[i] , count[array[i].getIdade()]--, i--)
      {
         this.setComparacoes(this.getComparacoes() + 1);
      }  
      ...
      //Desempatando a sigla usando outro algoritimo de ordenacao
      for(int i = 0; i < (n-1); i++)
      {
         int menor = i;
         ...
         for(int j = (i+1); j < n; j++)
         {
           if( (array[menor].getIdade( ) == array[j].getIdade( ) ) && 
            ( array[menor].getNome( ).compareTo( array[j].getNome( ) ) > 0 ) )
           {
             this.setComparacoes(this.getComparacoes() + 2);
             menor = j;
           }
      }
      swap(menor, i);
...
public class Q14
 {
     ...
     try
     {
        FileWriter matri = new FileWriter("matrícula_counting.txt");
