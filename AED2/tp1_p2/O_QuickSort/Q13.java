 ...
 class Lista 
{
   ...
   { 
      ...
      int i = esq , j = dir;
      LocalDate pivo = array[(dir+esq) / 2].getInicioMandato();

      while(i<=j)
      {
         while(array[i].getInicioMandato().compareTo(pivo) < 0 )
         {
            i++;
            this.setComparacoes(this.getComparacoes()+1);
         }

         while(array[j].getInicioMandato().compareTo(pivo) > 0)
         {
            j--;
            this.setComparacoes(this.getComparacoes()+1);
         }
...
public class Q13
 {
     ...
     try
     {
        FileWriter matri = new FileWriter("matrícula_quicksort.txt");
