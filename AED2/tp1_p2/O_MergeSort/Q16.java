 ...
 class Lista 
{
   ...
   // First subarray is arr[l..m] 
    ...
    // Uni dois subarrays formando em um array 
    void merge(Presidente arr[], int l, int m, int r) 
    { 
        //Pega o tamanho dos subarrays para ser fundido  
        int n1 = m - l + 1; 
        int n2 = r - m; 
  
        // cria arrays temporarios
        Presidente L[] = new Presidente [n1]; 
        Presidente R[] = new Presidente [n2]; 
        ...
        // Indice incial do primeiro e segundo subarrays 
        int i = 0, j = 0; 
        ...
        while (i < n1 && j < n2) 
        {
            if (L[i].getFimMandato().compareTo(R[j].getFimMandato()) <0) 
            {   
                //MyIO.println(">>>>"+R[j].getFimMandato());
                this.setComparacoes(this.getComparacoes()+1);
                this.setMovimentacoes(this.getMovimentacoes()+1);
                arr[k] = L[i]; 
                i++; 
            }
            else if ((L[i].getFimMandato().compareTo(R[j].getFimMandato( )) == 0 ) && 
            ( L[i].getNome( ).compareTo( R[j].getNome( ) ) > 0 )) 
            {
               arr[k] = L[i];
               this.setComparacoes(this.getComparacoes()+1);
               this.setMovimentacoes(this.getMovimentacoes()+1); 
    ...
    // ira ordernar o array formado  
    void sort(Presidente arr[], int l, int r) 
    { 
        if (l < r) 
        { 
...
public class Q16
 {
     ...
     { 
      ...
      lista.sort(lista.array, 0 , lista.n-1); 
     ...
     try
     {
        FileWriter matri = new FileWriter("matrícula_mergeSort.txt");
